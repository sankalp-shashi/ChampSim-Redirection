...
uint32_t CACHE::lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
    ...
    //if(flag==0){//it means that now replacement will have to be done.
    //    return -1;
    //}
    // LRU victim
    if (way == NUM_WAY) {
...
void CACHE::lru_update(uint32_t set, uint32_t way) //this function updates the array for the values that are least recently used.
{
