...
class CACHE : public MEMORY {
  public:
    ...
    uint8_t cache_type;
    //int good;
    std::map <int,int> eviction_count_map_cache; //this is storing how many evictions happened in a particular set.
    int total_evictions;
    std::map <uint64_t,uint32_t> mapping_set;
    std::map <uint64_t,uint32_t> already_there_mapping_set;
    vector<pair<int,int>> sorted_eviction_count_vector_cache;
    std::map <int,int> sorted_sets_map;
    int mapping_constant=0;
    int check_warmup=0;
    // prefetch stats
    ...
    //void sorted_eviction_count_vector();
