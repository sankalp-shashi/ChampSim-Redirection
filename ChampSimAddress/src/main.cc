...
void printHotAddresses(CACHE *cache)
{
//	cout << "HOT" << endl;
//	for (auto it = cache->hot_addresses.begin(); it != cache->hot_addresses.end(); it++)
//	{
//		cout << *it << endl;
//	}
}
...
void createHotAddress(CACHE *cache)
{
	cache->hot_addresses.clear();
	uint32_t blockCount = cache->miss_count.size();
	for (auto it = cache->miss_count.begin(); it != cache->miss_count.end(); it++)
	{
		if (it->second > 2*cache->total_miss_count/blockCount) cache->hot_addresses.insert(it->first);
	}
	printHotAddresses(cache);
}
...
int main(int argc, char** argv)
{
    ...
    while (run_simulation) {
		...
		if (warmup_complete[i]) instruction_count++;
            	if (warmup_complete[i] && instruction_count - previous_instruction_count >= 1000000)
		{
			previous_instruction_count = instruction_count;
			createHotAddress(&uncore.LLC);
		}
