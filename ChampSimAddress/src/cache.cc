...
uint8_t CACHE::isHot(uint64_t address)
{
	address /= 100000;
	for (auto it = hot_addresses.begin(); it != hot_addresses.end(); it++)
	{
		if (address == *it) return 1;
	}
	return 0;
}
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (cache_type == IS_LLC) {
        	eviction_count[set]++;
        	if (warmup_complete[fill_cpu] && hot_addresses.size())
        	{
        		uint32_t cold_way,counter=0;
			do
        		{
        			if (counter>=NUM_WAY-1) break;
        			counter++;
				for (cold_way = 0; cold_way < NUM_WAY; cold_way++)
				{
					if (block[set][cold_way].valid==false) break;
				}
				if (cold_way == NUM_WAY)
				{
					for (cold_way = 0; cold_way < NUM_WAY; cold_way++)
					{
						if (block[set][cold_way].lru == NUM_WAY-1) break;
					}
				}
				if (cold_way == NUM_WAY) cold_way--;
				lru_update(set,cold_way);
        		}while (isHot(block[set][cold_way].address));
        		
        		block[set][cold_way] = block[set][way];
        		block[set][cold_way].lru = 0;
        	}
        	else
			...
			if((WQ.entry[index].fill_l1d) && (MSHR.entry[mshr_index].fill_l1d != 1))
			  {
                ...
                if (cache_type == IS_LLC) {
                	eviction_count[set]++;
		       	if (warmup_complete[writeback_cpu] && hot_addresses.size())
				{
					uint32_t cold_way,counter;
					do
					{
						if (counter>=NUM_WAY-1) break;
						counter++;
...
int CACHE::check_hit(PACKET *packet)
{
	...
	if (match_way < 0 && warmup_complete[packet->cpu] && cache_type == IS_LLC)
	{
		miss_count[packet->address/100000]++;
		total_miss_count++;
	}
    return match_way;
}
