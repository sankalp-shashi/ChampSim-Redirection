...
class CACHE : public MEMORY {
  public:
	...
	std::set<uint64_t> hot_addresses;
	std::map<uint64_t, uint32_t> miss_count;
	uint64_t total_miss_count;
	uint32_t eviction_count[LLC_SET];
    ...
    // functions
	uint8_t isHot(uint64_t address);
