...
int previous_miss_count=0;
void CACHE::redirect(uint32_t set,uint32_t way,uint32_t cold_set,uint32_t cold_way)/*----redirect function which remaps the hot block to the corresponding cold block*/
{
  set_map.erase(block[cold_set][cold_way].address);
  block[cold_set][cold_way]=block[set][way]; //----cold block calculated in handle fill
  set_map[block[set][way].address]=cold_set;//---- new set address for the hot block
}
        ...
        if (cache_type == IS_LLC) {
          if(warmup_complete[fill_cpu])
          {
           total_miss_count++;
            ...
            sort(eviction_count.begin(),eviction_count.end());// sorts the vector of pairs containing the eviction count and corresponding sets
            uint32_t curr_setindex;// index of current set
            uint32_t cold_set;
            for(int i=0;i<2048;i++)
            {
              if(eviction_count[i].second==set)
              {
                curr_setindex=i;//----we calculate the index of the current set 
                break;
              }
            }
    //      if(curr_setindex>=1024)
          {
            cold_set=eviction_count[2047-curr_setindex].second;//---- we find the cold set corresponding to the current set  i.e. 7th hottest set is mapped to 7th coldest set
              uint32_t cold_way=llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, cold_set, block[cold_set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
              redirect(set,way,cold_set,cold_way);//----calling remappinf func
            previous_miss_count=total_miss_count;
             eviction_count[2047-curr_setindex].first++;//----increase eviction count of previous correspondingcold set
          }
     //     else
     //     {
     //       eviction_count[curr_setindex].first++;
    //    }
...
{
    uint32_t or_set=(uint32_t) (address & ((1 << lg2(NUM_SET)) - 1)); 
    if(set_map[address]==0) set_map[address]=or_set;
    return set_map[address];
}
