...
class CACHE : public MEMORY {
  public:
    ...
    vector<pair<uint32_t,uint32_t>> eviction_count;
    uint32_t cold_set;
    std::unordered_map<uint64_t,uint32_t> set_map;
    void redirect(uint32_t hot_set,uint32_t hot_way,uint32_t cold_set, uint32_t cold_way );
