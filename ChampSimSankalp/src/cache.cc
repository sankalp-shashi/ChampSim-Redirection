...
/*-------------------------------------------------------- MODIFIED CODE START ------------------------------------------------------------*/
           ...
           {
		uint32_t cold_set, cold_way;
           	for (uint32_t i = 0; i < LLC_SET; i++)
		{
			if (eviction_count[cold_set] > eviction_count[i]) cold_set = i;
		}
		{   	
		   	for (cold_way = 0; cold_way != NUM_WAY; cold_way++)
		   	{
				if (block[cold_set][cold_way].valid==false)
				{
					break;
				}
			}
			...
			{
				for (cold_way = 0; cold_way < NUM_WAY; cold_way++)
					if (block[cold_set][cold_way].lru==NUM_WAY-1) break;
			}
			if (cold_way == NUM_WAY) cold_way--;
			lru_update(cold_set,cold_way);
			set_map[block[set][way].address] = cold_set;
			block[cold_set][cold_way] = block[set][way];
			block[cold_set][cold_way].lru = 0;
		}
//		block[cold_set][cold_way].lru == NUM_WAY-1;
           	eviction_count[cold_set]++;
           }
        }
        else
/*-------------------------------------------------------=- MODIFIED CODE END -------------------------------------------------------------*/
...
/*-------------------------------------------------------- MODIFIED CODE START ------------------------------------------------------------*/		
		   if (warmup_complete[writeback_cpu]) 
		   {
			uint32_t cold_set, cold_way;
		   	for (uint32_t i = 0; i < LLC_SET; i++)
			{
				if (eviction_count[i] < eviction_count[cold_set]) cold_set = i;
			}
		   	for (cold_way = 0; cold_way != NUM_WAY; cold_way++)
		   	{
				if (block[cold_set][cold_way].valid==false)
...
{
/*-------------------------------------------------------- MODIFIED CODE START ------------------------------------------------------------*/
	uint32_t original_index = (address & ((1 << lg2(NUM_SET)) - 1));
	if (set_map[address]==0) set_map[address] = original_index;
